...
namespace BaccaratEngine
{
	...
	public sealed class BitHelper
	{
			...
			int pDestIndex, int pLength )
		{
			...
			if ( pLength < 0 || pLength + pDestIndex - 1 > BIT_SIZE_BYTE )
			{
				throw new ArgumentException( "End bit is out of range.", "pLength" );
			}
			...
			int lPosition = 1;
			for ( int i = pDestIndex; ( i < pLength + pDestIndex ); i++ )
			{
		...
		public static byte TurnOnOffBit( byte pInput, int pPosition, bool pOn )
		{
			if ( pPosition > BIT_SIZE_BYTE )
			{
				throw new ArgumentException( "Position out of range", "pPosition" );
			...
			int pDestIndex, int pLength )
		{
			...
			if ( pLength < 0 || pLength + pDestIndex - 1 > BIT_SIZE_SHORT )
			{
				throw new ArgumentException( "End bit is out of range.", "pLength" );
			}
			pSource = GetBits( pSource, pSourceIndex, pLength );
		...
		public static short TurnOnOffBit( short pInput, int pPosition, bool pOn )
		{
			if ( pPosition > BIT_SIZE_SHORT )
			{
				throw new ArgumentException( "Position out of range", "pPosition" );
		...
		/// <param name="pLength">Length to read.</param>
		/// <returns></returns>
		public static int SetBits( int pDest, int pSource, int pSourceIndex, int pDestIndex, int pLength )
		{
			...
			if ( pLength < 0 || pLength + pDestIndex - 1 > BIT_SIZE_INT )
			{
				throw new ArgumentException( "End bit is out of range.", "pLength" );
			}
			pSource = GetBits( pSource, pSourceIndex, pLength );
		...
		/// <param name="pInput">The input value</param>
		/// <returns></returns>
		public static int SizeOf( uint pInput )
		{
			int iRetval = 0;
			if ( pInput == 0 )
			{
		...
		/// <param name="pStart">The start position.</param>
		/// <returns></returns>
		public static uint GetBits( uint pInput, int pStartIndex )
		{
			return GetBits( pInput, pStartIndex, BIT_SIZE_INT, false );
		}
		...
		/// <param name="pShift">if set to <c>true</c> [p shift].</param>
		/// <returns></returns>
		public static uint GetBits( uint pInput, int pStartIndex, bool pShift )
		{
			return GetBits( pInput, pStartIndex, BIT_SIZE_INT, pShift );
		}
		...
		/// <param name="pLength">Length of the p.</param>
		/// <returns></returns>
		public static uint GetBits( uint pInput, int pStartIndex, int pLength )
		{
			return GetBits( pInput, pStartIndex, pLength, false );
		}
		...
		/// <param name="pEnd"></param>
		/// <returns></returns>
		public static uint GetBits( uint pInput, int pStartIndex, int pLength, bool pShift )
		{
			uint lRetval = 0;
			int lSize = 0,lTemp = 0;
			int lPosition = 1;
			...
			for ( int i = pStartIndex; ( i < pLength + pStartIndex ) && ( lPosition <= lSize ); i++ )
			{
				...
				if ( ( pInput & lTemp ) == lTemp )
				{
					uint shifted = (uint)( 1 << ( lPosition - 1 ) );
					lRetval |= shifted;
				}
				lPosition++;
			}
		...
		/// <param name="pSourceIndex">Index of the p source.</param>
		/// <returns></returns>
		public static uint SetBits( uint pDest, uint pSource, int pSourceIndex )
		{
			return SetBits( pDest, pSource, pSourceIndex, 0, BIT_SIZE_INT );
		}
		...
		/// <param name="pLength">Length of the p.</param>
		/// <returns></returns>
		public static uint SetBits( uint pDest, uint pSource, int pSourceIndex, int pLength )
		{
			return SetBits( pDest, pSource, pSourceIndex, 0, pLength );
		}
		...
		/// <param name="pLength">Length to read.</param>
		/// <returns></returns>
		public static uint SetBits( uint pDest, uint pSource, int pSourceIndex, int pDestIndex, int pLength )
		{
			int  lTemp1 = 0;
			if ( pSourceIndex < 1 || pSourceIndex > BIT_SIZE_INT )
			{
		...
		/// 	<c>true</c> if [is bit set] [the specified p input]; otherwise, <c>false</c>.
		/// </returns>
		public static bool IsBitSet( uint pInput, int pPosition )
		{
			return GetBits( pInput, pPosition, 1, false ) == 1;
		}
		...
		/// <param name="pPosition"></param>
		/// <returns></returns>
		public static uint ChangeBit( uint pInput, int pPosition )
		{
			if ( pPosition > BIT_SIZE_INT )
			{
				throw new ArgumentException( "Position out of range", "pPosition" );
			}
			return pInput ^= (uint)( 1 << ( pPosition - 1 ) );
		}
		...
		/// <param name="pOn">if set to <c>true</c> [p on].</param>
		/// <returns></returns>
		public static uint SetBit( uint pInput, int pPosition, bool pOn )
		{
			if ( pPosition > BIT_SIZE_INT )
			{
				throw new ArgumentException( "Position out of range", "pPosition" );
			...
			if ( pOn && !lIsSet || pOn && lIsSet )
			{
				pInput ^= (uint)( 1 << ( pPosition - 1 ) );
			}
			return pInput;
		}

		public static uint TurnOnOffBit( uint pInput, int pPosition, bool pOn )
		{
			if ( pPosition > BIT_SIZE_INT )
			{
				throw new ArgumentException( "Position out of range", "pPosition" );
		...
		/// <param name="pLength">Length to read.</param>
		/// <returns></returns>
		public static long SetBits( long pDest, long pSource, int pSourceIndex, int pDestIndex, int pLength )
		{
			long lTemp1 = 0;
			...
			if ( pLength < 0 || pLength + pDestIndex - 1  > BIT_SIZE_LONG )
			{
				throw new ArgumentException( "End bit is out of range.", "pLength" );
			}
		...
		public static long TurnOnOffBit( long pInput, int pPosition, bool pOn )
		{
			if ( pPosition > BIT_SIZE_LONG )
			{
				throw new ArgumentException( "Position out of range", "pPosition" );
